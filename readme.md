...
- **Design System**: Dark premium theme with CSS custom properties in `:root` of `styles.css`.
- **Navigation**: Desktop uses a persistent sidebar; tablet (≤1024px) uses a compact icon-rail; mobile (≤768px) uses a hamburger-triggered off-canvas drawer.
- **Responsive Spacing**: The `--page-padding` CSS variable controls `.main` padding and `.top-bar` bleed margins. Override it in media queries—never hardcode padding values.
- **Mobile Drawer**: Managed by `bindMobileNav()` in `public/js/ui.js`. The `inert` attribute is only applied on mobile viewports (≤768px) when the drawer is closed.
- **Agent Instructions**: See `AGENTS.md` for recurring rules and known pitfalls.
